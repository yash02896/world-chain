...
mod tests {
    ...
    use reth_transaction_pool::validate::{ensure_intrinsic_gas, ForkTracker};
    use reth_transaction_pool::{
        blobstore::InMemoryBlobStore, validate::EthTransactionValidatorBuilder,
        EthPooledTransaction, TransactionOrigin, TransactionValidationOutcome,
    };
    use reth_transaction_pool::{
        CoinbaseTipOrdering, Pool, PoolTransaction as _, TransactionPool as _,
    };
    ...
    fn valid_proof() -> semaphore::protocol::Proof {
        let mut hello = *b"identity";
        create_proof(&mut hello, b"appId", b"xxx", 30)
    }
    ...
    #[test]
    fn validate_non_pbh_tx() {
        // let eth_tx = EthPooledTransaction::new(TransactionSignedEcRecovered::default(), 0);
        ...
        let validator = world_chain_validator();
        let res = validator.validate_one(TransactionOrigin::External, tx);
        panic!("{:?}", res);
    ...
    fn test_one() {
        ...
        test_end_to_end(&mut hello, b"appId", b"xxx", 30);
    }
